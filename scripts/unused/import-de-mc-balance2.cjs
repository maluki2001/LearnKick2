...
function convertToDbSchema(question, index, schoolId, userId) {
  return {
    ...
    correct_answer: question.answers[question.correctIndex],
    explanation: null,
    lehrplan21_topic: question.lehrplan21Code,
    school_id: schoolId,
    created_by: userId,
    time_limit: question.timeLimit || 18000,
    tags: question.tags || [question.subject, `grade-${question.grade}`, 'swiss-german', 'mc-balance2'],
  };
}
...
async function importQuestions() {
  ...
  try {
    console.log('📖 Reading questions from de-mc-balance2.json...');
    const questionsPath = path.join(__dirname, 'de-mc-balance2.json');
