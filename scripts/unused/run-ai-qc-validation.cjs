 ...
 * Uses Claude API to verify factual accuracy of questions and answers
 */
...
Type: ${q.type}
Question: ${q.question}
${q.type === 'multiple-choice' ? `Options: ${q.answers.join(' | ')}` : ''}
...
/**
 * Validate a batch of questions using Claude API
 */
...
async function runAIQC() {
  ...
  try {
    ...
    console.log('   Using Claude API for Factual Accuracy Verification');
    console.log('='.repeat(60));
    console.log('');
    ...
    if (MAX_QUESTIONS) {
      query = query.replace('ORDER BY', `LIMIT ${MAX_QUESTIONS} ORDER BY`);
    ...
    if (qcResults.errors.length > 0) {
      ...
      qcResults.errors.slice(0, 20).forEach((e, i) => {
        ...
        console.log(`\n${i + 1}. [${q?.subject}/${q?.grade}] ${q?.question?.substring(0, 60)}...`);
